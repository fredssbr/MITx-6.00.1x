...
def updateHand(hand, word):
    ...
    for char in word:
        updatedHand[char] = updatedHand.get(char,0) - 1
    return updatedHand
...
def isValidWord(word, hand, wordList):
    ...
    if word in wordList:
        for char in word:
            if char in updatedHand and updatedHand.get(char,0)>0:
                updatedHand[char] = updatedHand.get(char,0) - 1
            else:
                isValid = False
                break
...
def calculateHandlen(hand):
    ...
    returns: integer
    """
    #Sums only when the ocurrency of the letter > 0
    length = 0
    for key in hand:
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(updatedHand) > 0:
        ...
        if word == ".":
            ...
            if isValidWord(word, updatedHand, wordList):
...
def playGame(wordList):
    ...
    1) Asks the user to input 'n' order 'r' or 'e'.
